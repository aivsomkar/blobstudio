...
/**
 * Rasterises `<g transform=fit>{markup}</g>` over the face box and builds the field.
 * Async because the browser only rasterises SVG through an Image load.
 */
export async function buildSdf(markup: string, fit: string, size = 256): Promise<Sdf> {
  const mask = await rasterise(markup, fit, size)
  return fieldFromMask(mask, size)
}

async function rasterise(markup: string, fit: string, size: number): Promise<Uint8Array> {
  // Solid black on transparent: we only care about coverage, so alpha is the whole signal.
  const svg =
    `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" ` +
    `viewBox="0 0 ${FACE_BOX} ${FACE_BOX}">` +
    `<g${fit ? ` transform="${escapeAttr(fit)}"` : ''} fill="#000" stroke="none">${markup}</g>` +
    `</svg>`
  ...
  try {
    ...
    if (!ctx) throw new RasterError('Could not get a 2D canvas context.')
    ...
    try {
      ...
      // Only happens if the SVG pulled in a cross-origin resource; the importer strips those.
      throw new RasterError('That SVG referenced external content and could not be measured.')
    }
...
function loadImage(url: string): Promise<HTMLImageElement> {
  return new Promise((resolve, reject) => {
    ...
    image.onerror = () => reject(new RasterError('The browser could not render that SVG.'))
    image.src = url
  })
}
